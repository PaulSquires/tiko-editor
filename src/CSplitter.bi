...
'
'    A self-contained, per-instance control in the same vein as CVScrollBar/CStatusBar: it
...
'          CSPLITTER_VERTICAL   -> the bar's x (a vertical bar splitting left/right panes)
'          CSPLITTER_HORIZONTAL -> the bar's y (a horizontal bar splitting top/bottom panes)
...
#define CSPLITTER_HOTTRACK_MS    100
...
' coordinates; nPhase is one of the CSPLITTER_PHASE_* values. The control has already moved
' itself -- lay out the two panes and nothing else.
'
' NOT fired for CSplitter_SetPos / CSplitter_SetRange: programmatic setters are silent, so a
...
type CSPLITTER
    ...
    nOrientation   as integer = CSPLITTER_VERTICAL
    ...
    ' CStatusBar's hPanelFont/HFONT. See C:\dev\Learnings.md.
...
function CSPLITTER.ClampPos( byval n as integer ) as integer
...
' nPos is measured in.
function CSPLITTER.WindowRectInParent() as RECT
    dim as RECT rc
...
function CSPLITTER.BarEdgeFromWindow() as integer
    dim as RECT rc = this.WindowRectInParent()
    if this.nOrientation = CSPLITTER_VERTICAL then return rc.left
    return rc.top
end function
...
function CSPLITTER.CursorAxisInParent() as integer
    dim as POINT pt
    GetCursorPos( @pt )
    if this.hParent then ScreenToClient( this.hParent, @pt )
    if this.nOrientation = CSPLITTER_VERTICAL then return pt.x
    return pt.y
end function
...
sub CSPLITTER.Notify( byval nPhase as integer )
...
'
' THE CONTROL HANDLE
'   Every CSplitter_* function takes the handle returned by CSplitter_Create().
'
...
'
'   CSplitter_SetPos    clamps, moves the bar, repaints -- and fires NOTHING. This is the
'                       host telling the control where it put the splitter, not the user
'                       moving it.
'   CSplitter_SetRange  the limits the drag clamps to. The control does NOT derive them
...
'   CSplitter_GetPos    always agrees with where the bar actually is: the control re-syncs
'                       from its own window rect whenever the HOST moves it.
' ----------------------------------------------------------------------------------------
declare function CSplitter_GetPos( byval hSplit as HWND ) as integer
declare sub      CSplitter_SetPos( byval hSplit as HWND, byval nPosition as integer )
