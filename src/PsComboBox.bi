''
''  PsComboBox.bi  --  owner-drawn dropdown selector (button + checkmarked list)
''

#pragma once

' BOTH dependencies are included HERE rather than left to the call site. PsListBox.bi names
...
'
' A host that already vendors PsPopupMenu.bi gets ONE copy: #include once dedupes by resolved
' path, which is how tiko already shares a single copy between PsTextBox and PsMenuBar.
'
...
#define PSCOMBOBOX_HOTTRACK_MS    100
...
#define PSCOMBOBOX_DEFAULT_PADLEFT     12
#define PSCOMBOBOX_DEFAULT_PADRIGHT    10
#define PSCOMBOBOX_DEFAULT_PADTOP       6
#define PSCOMBOBOX_DEFAULT_PADBOTTOM    6
...
#define PSCOMBOBOX_DEFAULT_TEXTGAP      8
#define PSCOMBOBOX_DEFAULT_CHEVRONW     9
#define PSCOMBOBOX_DEFAULT_CHEVRONH    12
' Vertical air between the up chevron and the down chevron, inside CHEVRONH.
#define PSCOMBOBOX_DEFAULT_CHEVRONGAP   3
...
#define PSCOMBOBOX_DEFAULT_CURVATURE   12
' The button reads BORDERLESS out of the box (see PSCOMBOBOX_COLORS), so a border thickness
' of 1 costs nothing until a host sets a contrasting BorderColor. Zero disables it outright.
#define PSCOMBOBOX_DEFAULT_BORDERTHICK  1     ' not DPI-scaled
#define PSCOMBOBOX_DEFAULT_FOCUSGAP     2
#define PSCOMBOBOX_DEFAULT_FOCUSTHICK   1     ' not DPI-scaled
...
'
' Practical consequence: in this mode, either turn on PsComboBox_SetAutoSize, or re-place the
...
' the matching BackColorXxx -- the same trick PsToggle uses for its ON pill and PsSelectBar for
...
' and the items are drawn by PsPopupMenu inside the dropdown -- PsComboBox never lays them out,
...
'
' This callback owns the button only. The DROPDOWN is a PsPopupMenu and is painted by its own
' PM_PaintItemCallbackSub -- reach it with PsComboBox_GetListHandle().
'
...
' Draw with the SAME font you handed to PsComboBox_SetFont: the button width was measured with
...
'
' Like PsToggle -- and unlike the mouse-only siblings -- this control hands over the FOCUS and
...
' PsComboBox_GetCurSel() = idxNew.
'
' The programmatic PsComboBox_SetCurSel does NOT fire this (Win32's CB_SETCURSEL / CBN_SELCHANGE
...
'
' Fired for USER interaction and for the programmatic PsComboBox_DropDown / PsComboBox_CloseUp
...
type PSCOMBOBOX
    ...
    items(any)        as PSCOMBOBOX_ITEM
    itemCount         as long = 0
    ' --- Selection and state ---
    ' nCurSel is a MODEL index, or -1 for "nothing selected" (which is legal here, unlike
    ' PsSelectBar's always-one contract -- a picker that has not been answered yet is a real
    ...
    ' re-derived from PSCOMBOBOX_COLORS every time PsComboBox_SetColors runs, so theming the
    ' button themes the list for free. PsComboBox_SetListColors sets this and the derivation
    ' stands down permanently -- an explicit choice is never overwritten.
    bListColorsCustom as boolean = false
    ' Set by PsComboBox_FilterMessage when a click lands on THIS button while the list is
    ' open, and consumed by the very next WM_LBUTTONDOWN. PsPopupMenu's filter dismisses on
    ...
    nPadLeft          as long = PSCOMBOBOX_DEFAULT_PADLEFT
    nPadRight         as long = PSCOMBOBOX_DEFAULT_PADRIGHT
    nPadTop           as long = PSCOMBOBOX_DEFAULT_PADTOP
    nPadBottom        as long = PSCOMBOBOX_DEFAULT_PADBOTTOM
    nTextGap          as long = PSCOMBOBOX_DEFAULT_TEXTGAP
    nChevronWidth     as long = PSCOMBOBOX_DEFAULT_CHEVRONW
    nChevronHeight    as long = PSCOMBOBOX_DEFAULT_CHEVRONH
    nChevronGap       as long = PSCOMBOBOX_DEFAULT_CHEVRONGAP
    nChevronThick     as long = 1          ' NOT DPI-scaled here (see the note on the setter)
    nCurvature        as long = PSCOMBOBOX_DEFAULT_CURVATURE   ' ellipse DIAMETER
    nBorderThick      as long = PSCOMBOBOX_DEFAULT_BORDERTHICK ' NOT DPI-scaled
    nFocusGap         as long = PSCOMBOBOX_DEFAULT_FOCUSGAP
    nFocusThick       as long = PSCOMBOBOX_DEFAULT_FOCUSTHICK  ' NOT DPI-scaled
    ...
    declare function GetItem( byval idx as long ) as PSCOMBOBOX_ITEM ptr
    declare function AddItemSlot() as PSCOMBOBOX_ITEM ptr
    declare function InsertItemSlot( byval idx as long ) as PSCOMBOBOX_ITEM ptr
...
' It therefore contributes to the IDEAL SIZE (PsToggle's rule).
function PSCOMBOBOX.RingPad() as long
...
function PSCOMBOBOX.IsTextVisible() as boolean
...
function PSCOMBOBOX.GetCount() as long
    return this.itemCount
end function

function PSCOMBOBOX.IsValidItem( byval idx as long ) as boolean
    return (idx >= 0) andalso (idx < this.itemCount)
end function

function PSCOMBOBOX.GetItem( byval idx as long ) as PSCOMBOBOX_ITEM ptr
...
function PSCOMBOBOX.DisplayText( byref bIsPlaceholder as boolean ) as DWSTRING
...
' PsPopupMenu.NextSelectable, which wraps because menu semantics want that. Win32's own combobox
...
function PSCOMBOBOX.NextSelectable( byval nFrom as long, byval nDir as long ) as long
...
' auto-select the first item the way PsSelectBar does, because -1 is a legal state here.
function PSCOMBOBOX.AddItemSlot() as PSCOMBOBOX_ITEM ptr
...
function PSCOMBOBOX.InsertItemSlot( byval idx as long ) as PSCOMBOBOX_ITEM ptr
...
function PSCOMBOBOX.RemoveItemAt( byval idx as long ) as boolean
...
' client area at all, and PsComboBox_GetIdealSize is exactly what a host calls to decide how big
' to make the control in the FIRST place. Returning 0 until it had already been sized would be
' a chicken-and-egg trap (PsSelectBar's nTotalWidth and PsToggle's GetIdealSize both dodge it,
...
' squeezed (PsIconPanel's and PsTabBar's rule) -- the chevron keeps its size and stays pinned to
...
'
' THE DROPDOWN IS A PsPopupMenu, USED AS-IS
...
'
'     1. THE CHECKMARK SITS IN A LEFT GUTTER. PsPopupMenu's nCheckColWidth (30) and
...
'        Making the check right-aligned would mean patching PsMenuBar, which was ruled out.
'
'     2. THERE IS NO SCROLLING. PsPopupMenu bottom-clamps to the work area; a list taller than
...
'           if PsComboBox_FilterMessage( hCombo, @uMsg ) then continue do
'           if IsDialogMessage( hMain, @uMsg ) then continue do
'           TranslateMessage( @uMsg ) : DispatchMessage( @uMsg )
'       loop
'
'   Call it once per combobox, ahead of IsDialogMessage. See PsComboBox_FilterMessage.
'
' IT IS FOCUSABLE
'   WS_TABSTOP, real focus tracking, a painted focus ring, and Win32 combobox key handling.
'   PsToggle is the only other focusable control in the family; everything else is mouse-only.
...
'
' THE CONTROL HANDLE
'   Every PsComboBox_* function takes the handle returned by PsComboBox_Create(). It is a real
...
'   The control is created zero-sized: size it with PsComboBox_GetIdealSize and position it
'   with SetWindowPos() -- or turn on PsComboBox_SetAutoSize and let it size itself.
' ----------------------------------------------------------------------------------------
declare function PsComboBox_Create( byval hWndParent as HWND, byval CtrlID as long ) as HWND
...
' ----------------------------------------------------------------------------------------
declare function PsComboBox_AddItem( byval hCombo as HWND, byval Text as DWSTRING, byval id as long = 0, byval itemData as integer = 0 ) as long
declare function PsComboBox_InsertItem( byval hCombo as HWND, byval idx as long, byval Text as DWSTRING, byval id as long = 0, byval itemData as integer = 0 ) as long
declare function PsComboBox_DeleteItem( byval hCombo as HWND, byval idx as long ) as boolean
declare sub      PsComboBox_Clear( byval hCombo as HWND )
declare function PsComboBox_GetCount( byval hCombo as HWND ) as long
declare function PsComboBox_IsValidItem( byval hCombo as HWND, byval idx as long ) as boolean
declare function PsComboBox_GetItemText( byval hCombo as HWND, byval idx as long ) as DWSTRING
declare function PsComboBox_SetItemText( byval hCombo as HWND, byval idx as long, byval Text as DWSTRING ) as boolean
declare function PsComboBox_GetItemID( byval hCombo as HWND, byval idx as long ) as long
declare function PsComboBox_SetItemID( byval hCombo as HWND, byval idx as long, byval id as long ) as boolean
declare function PsComboBox_GetItemData( byval hCombo as HWND, byval idx as long ) as integer
declare function PsComboBox_SetItemData( byval hCombo as HWND, byval idx as long, byval itemData as integer ) as boolean
declare function PsComboBox_GetItemEnabled( byval hCombo as HWND, byval idx as long ) as boolean
declare function PsComboBox_SetItemEnabled( byval hCombo as HWND, byval idx as long, byval bEnabled as boolean ) as boolean
' Index of the FIRST item carrying this id / this exact text (case-insensitive), or -1.
declare function PsComboBox_FindItemByID( byval hCombo as HWND, byval id as long ) as long
declare function PsComboBox_FindItemByText( byval hCombo as HWND, byval Text as DWSTRING ) as long
...
' placeholder, or "".
' ----------------------------------------------------------------------------------------
declare function PsComboBox_GetCurSel( byval hCombo as HWND ) as long
declare function PsComboBox_SetCurSel( byval hCombo as HWND, byval idx as long ) as boolean
declare function PsComboBox_GetText( byval hCombo as HWND ) as DWSTRING
...
' ----------------------------------------------------------------------------------------
declare function PsComboBox_GetEnabled( byval hCombo as HWND ) as boolean
declare sub      PsComboBox_SetEnabled( byval hCombo as HWND, byval isEnabled as boolean )
declare function PsComboBox_GetFocused( byval hCombo as HWND ) as boolean
declare function PsComboBox_IsDroppedDown( byval hCombo as HWND ) as boolean
declare function PsComboBox_DropDown( byval hCombo as HWND ) as boolean
declare sub      PsComboBox_CloseUp( byval hCombo as HWND )
declare sub      PsComboBox_Refresh( byval hCombo as HWND )
...
'     stroke weight. NOTE the stroke is DPI-scaled by PsBufferPaint.PaintLine itself, unlike
...
' ----------------------------------------------------------------------------------------
declare function PsComboBox_GetTextMode( byval hCombo as HWND ) as long
declare sub      PsComboBox_SetTextMode( byval hCombo as HWND, byval nTextMode as long )
declare function PsComboBox_GetShowText( byval hCombo as HWND ) as boolean
declare sub      PsComboBox_SetShowText( byval hCombo as HWND, byval bShowText as boolean )
declare function PsComboBox_GetAutoSize( byval hCombo as HWND ) as boolean
declare sub      PsComboBox_SetAutoSize( byval hCombo as HWND, byval bAutoSize as boolean )
declare sub      PsComboBox_GetPadding( byval hCombo as HWND, byref nLeft as long, byref nTop as long, byref nRight as long, byref nBottom as long )
declare sub      PsComboBox_SetPadding( byval hCombo as HWND, byval nLeft as long, byval nTop as long, byval nRight as long, byval nBottom as long )
declare function PsComboBox_GetTextGap( byval hCombo as HWND ) as long
declare sub      PsComboBox_SetTextGap( byval hCombo as HWND, byval nTextGap as long )
declare sub      PsComboBox_GetChevronSize( byval hCombo as HWND, byref nWidth as long, byref nHeight as long )
declare sub      PsComboBox_SetChevronSize( byval hCombo as HWND, byval nWidth as long, byval nHeight as long )
declare function PsComboBox_GetChevronGap( byval hCombo as HWND ) as long
declare sub      PsComboBox_SetChevronGap( byval hCombo as HWND, byval nGap as long )
declare function PsComboBox_GetChevronThickness( byval hCombo as HWND ) as long
declare sub      PsComboBox_SetChevronThickness( byval hCombo as HWND, byval nThickness as long )
declare function PsComboBox_GetCornerCurvature( byval hCombo as HWND ) as long
declare sub      PsComboBox_SetCornerCurvature( byval hCombo as HWND, byval nCurvature as long )
declare function PsComboBox_GetBorderThickness( byval hCombo as HWND ) as long
declare sub      PsComboBox_SetBorderThickness( byval hCombo as HWND, byval nThickness as long )
declare sub      PsComboBox_GetFocusRing( byval hCombo as HWND, byref nGap as long, byref nThickness as long )
declare sub      PsComboBox_SetFocusRing( byval hCombo as HWND, byval nGap as long, byval nThickness as long )
declare sub      PsComboBox_GetIdealSize( byval hCombo as HWND, byref nWidth as long, byref nHeight as long )
declare function PsComboBox_GetButtonRect( byval hCombo as HWND, byref rc as RECT ) as boolean
declare function PsComboBox_GetTextRect( byval hCombo as HWND, byref rc as RECT ) as boolean
declare function PsComboBox_GetChevronRect( byval hCombo as HWND, byref rc as RECT ) as boolean
declare function PsComboBox_GetVisualRect( byval hCombo as HWND, byref rc as RECT ) as boolean
...
' always agree.
'
' THE DROPDOWN'S OWN APPEARANCE goes through PsComboBox_GetListHandle(), which returns the
' internal PsPopupMenu HWND -- so every PsPopupMenu_Set* appearance call is available
...
' PsComboBox_EnsureList() first if you want to theme it up front.
'
...
' mark is placed by PsComboBox, from nCurSel.
' ----------------------------------------------------------------------------------------
declare sub      PsComboBox_GetColors( byval hCombo as HWND, byval pColors as PSCOMBOBOX_COLORS ptr )
declare sub      PsComboBox_SetColors( byval hCombo as HWND, byval pColors as PSCOMBOBOX_COLORS ptr )
declare function PsComboBox_GetFont( byval hCombo as HWND ) as HFONT
declare sub      PsComboBox_SetFont( byval hCombo as HWND, byval hTextFont as HFONT )
declare function PsComboBox_GetPlaceholderText( byval hCombo as HWND ) as DWSTRING
declare sub      PsComboBox_SetPlaceholderText( byval hCombo as HWND, byval Text as DWSTRING )
declare function PsComboBox_EnsureList( byval hCombo as HWND ) as HWND
declare function PsComboBox_GetListHandle( byval hCombo as HWND ) as HWND
declare sub      PsComboBox_SetListColors( byval hCombo as HWND, byval pColors as PSPOPUPMENU_COLORS ptr )
declare sub      PsComboBox_SetListItemHeight( byval hCombo as HWND, byval nItemHeight as long )
...
'                       is painted by PsPopupMenu; reach it through GetListHandle.
...
' ----------------------------------------------------------------------------------------
declare sub      PsComboBox_SetPaintCallback( byval hCombo as HWND, byval usersub as CBO_PaintCallbackSub )
declare sub      PsComboBox_SetMessageCallback( byval hCombo as HWND, byval userfunc as CBO_MessageCallbackFunc )
declare sub      PsComboBox_SetSelChangeCallback( byval hCombo as HWND, byval usersub as CBO_SelChangeCallbackSub )
declare sub      PsComboBox_SetDropDownCallback( byval hCombo as HWND, byval usersub as CBO_DropDownCallbackSub )
...
'
'                   It also carries this design's one trap, and handles it. PsPopupMenu's own
...
'
'   RunSelfTest   - geometry assertions, gated on the PSCOMBOBOX_SELFTEST env var. Call it
'                   from the host before the message loop.
' ----------------------------------------------------------------------------------------
declare function PsComboBox_FilterMessage( byval hCombo as HWND, byval pMsg as MSG ptr ) as boolean
declare sub      PsComboBox_RunSelfTest( byval hWndParent as HWND )
