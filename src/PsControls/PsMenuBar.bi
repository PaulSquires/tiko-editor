...
' The bar's dropdowns ARE PsPopupMenu windows; the dependency is one-way (PsPopupMenu
...
#define PSMENUBAR_HOTTRACK_MS    100

' Default horizontal inset applied to each side of an item's caption when measuring
' (tiko uses 6+6). DPI-scaled at Create; PsMenuBar_SetPadding takes raw pixels.
#define PSMENUBAR_DEFAULT_PADDING  6
...
type PSMENUBAR_ITEM
    ...
    wszText   as DWSTRING
    hDropDown as HWND            ' attached PsPopupMenu window. OWNED once attached: the
...
' PsMenuBar_OpenMenu). The hook for just-in-time state: flip enabled/checked on the
...
' Draw one bar item, INSTEAD OF the built-in painter. See PSMENUBAR_PAINTINFO.
type MB_PaintItemCallbackSub as sub( byval p as PSMENUBAR_PAINTINFO ptr )


type PSMENUBAR
    hWin              as HWND
    items(any)        as PSMENUBAR_ITEM
    itemCount         as long = 0
...
' ========================================================================================
'
' A MENU BAR: a horizontal strip of text items, each optionally owning a PsPopupMenu
...
' focus -- keyboard navigation arrives through PsMenuBar_FilterMessage, which the host
' MUST call in its message pump (see the contract in PsPopupMenu.bi; the bar's filter
...
' decides what Alt (or an accelerator like Alt+F) does and calls PsMenuBar_OpenMenu /
' PsMenuBar_SetActiveIndex to drive the bar (tiko routes Alt+F etc. through its
...
' host's. PsMenuBar_GetIdealWidth reports the items' total width for exactly that.
'
...
' ----------------------------------------------------------------------------------------
declare function PsMenuBar_Create( byval hWndParent as HWND, byval CtrlID as long ) as HWND

' ----------------------------------------------------------------------------------------
' Items. AddItem appends and returns the new index (or -1). SetDropDown attaches a
' PsPopupMenu as the item's dropdown -- OWNERSHIP TRANSFERS; attaching over an existing
' dropdown destroys the old one; 0 detaches without destroying. Programmatic mutators
' are SILENT.
' ----------------------------------------------------------------------------------------
declare function PsMenuBar_AddItem( byval hMenuBar as HWND, byval Text as DWSTRING, byval id as long ) as long
declare function PsMenuBar_GetItemCount( byval hMenuBar as HWND ) as long
declare function PsMenuBar_GetItemText( byval hMenuBar as HWND, byval idx as long ) as DWSTRING
declare function PsMenuBar_SetItemText( byval hMenuBar as HWND, byval idx as long, byval Text as DWSTRING ) as boolean
declare function PsMenuBar_SetDropDown( byval hMenuBar as HWND, byval idx as long, byval hPopup as HWND ) as boolean
declare function PsMenuBar_GetDropDown( byval hMenuBar as HWND, byval idx as long ) as HWND
...
' ----------------------------------------------------------------------------------------
declare function PsMenuBar_GetItemRect( byval hMenuBar as HWND, byval idx as long, byref rc as RECT ) as boolean
declare function PsMenuBar_GetIdealWidth( byval hMenuBar as HWND ) as long
declare function PsMenuBar_GetPadding( byval hMenuBar as HWND ) as long
declare sub      PsMenuBar_SetPadding( byval hMenuBar as HWND, byval nPadding as long )
...
' ----------------------------------------------------------------------------------------
declare function PsMenuBar_OpenMenu( byval hMenuBar as HWND, byval idx as long, byval bSelectFirst as boolean = false ) as boolean
declare sub      PsMenuBar_CloseMenu( byval hMenuBar as HWND )
declare function PsMenuBar_IsMenuOpen( byval hMenuBar as HWND ) as boolean
declare function PsMenuBar_GetActiveIndex( byval hMenuBar as HWND ) as long
declare function PsMenuBar_SetActiveIndex( byval hMenuBar as HWND, byval idx as long ) as boolean

' ----------------------------------------------------------------------------------------
' Appearance. The font is borrowed (caller owns it) and is also the measuring font.
' ----------------------------------------------------------------------------------------
declare sub      PsMenuBar_SetColors( byval hMenuBar as HWND, byval pColors as PSMENUBAR_COLORS ptr )
declare sub      PsMenuBar_SetFont( byval hMenuBar as HWND, byval hBarFont as HFONT )
...
'                   End while a dropdown is open (wrapping the PsPopupMenu filter and
...
' ----------------------------------------------------------------------------------------
declare function PsMenuBar_FilterMessage( byval hMenuBar as HWND, byval pMsg as MSG ptr ) as boolean
declare sub      PsMenuBar_SetCommandCallback( byval hMenuBar as HWND, byval usersub as MB_CommandCallbackSub )
declare sub      PsMenuBar_SetInitPopupCallback( byval hMenuBar as HWND, byval usersub as MB_InitPopupCallbackSub )
declare sub      PsMenuBar_SetPaintItemCallback( byval hMenuBar as HWND, byval usersub as MB_PaintItemCallbackSub )
