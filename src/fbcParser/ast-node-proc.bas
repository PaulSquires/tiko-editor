...
private sub hProcFlush _
	...
	'' fbcParser: locals don't survive the deletion below - hand them to the
	'' symbol collector now
	collHarvestProcLocals( sym )
...
function astProcEnd( byval callrtexit as integer ) as integer
	...
	'' errors.. remove everything from hash and symbol tb
		...
		'' fbcParser: still harvest the locals - a broken proc should yield
		'' partial results, not nothing
		collHarvestProcLocals( sym )
		symbDelSymbolTb( @sym->proc.symtb, FALSE )
	end if
