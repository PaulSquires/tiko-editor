...
private sub hPrintErrMsg _
	...
	'' fbcParser: compose the message body for the collector first (the diag
	'' record carries file/line separately, so the filename prefix and the
	'' source-line echo are not captured), then optionally echo to the
	'' console - a DLL scan runs silent (see FBCP_OPTFLAG_CONSOLE).
	dim as string diagtext
	...
	collAddDiag( errnum, linenum, diagtext )

	if( collConsoleOutput( ) = FALSE ) then
		exit sub
	end if
