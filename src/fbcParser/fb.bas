...
sub fbInit _
	...
	errInit( )
	astInit( )
	'' Restored: was deleted in the early IR-removal experiment, but the
	'' parser drives ir.vtbl calls during cProgram(), so the vtable must be
	'' initialized. Phase 1 points this at a null backend instead.
	irInit( )
	...
	'' fbcParser: filenamehash OWNS the interned filename strings allocated
	'' by hAddIncFile (delstr = TRUE, upstream leaked them per compile);
	'' incfilehash/inconcehash only reference the same pointers, so their
	'' hashEnd must stay delstr = FALSE. fbEnd's hashEnd order doesn't
	'' matter: delstr = FALSE never dereferences the (freed) strings.
	hashInit( @env.filenamehash, FB_INITINCFILES, TRUE )
...
sub fbGlobalInit()
	...
	if( inited ) then
		strlistEnd(@env.predefines)
		strlistEnd(@env.preincludes)
		strlistEnd(@env.includepaths)
	end if
	inited = TRUE
...
'' fbcParser: stage an in-memory root source for the next fbCompile() calls
'' (stays set across #cmdline restarts within one scan; the scan entry point
'' clears it when done)
dim shared fbMemSource as const zstring ptr = NULL

sub fbSetMemSource( byval ptext as const zstring ptr )
	fbMemSource = ptext
end sub
...
sub fbCompile _
	...
	'' fbcParser: memory-backed root source? (fbc_compile_text) The name is
	'' virtual then - used for symbol attribution and same-dir #include
	'' resolution - and no file is opened or required to exist.
	env.inf.memtext = fbMemSource
	if( env.inf.memtext <> NULL ) then
		env.inf.memlen = len( *env.inf.memtext )
		env.inf.mempos = 1
		'' tolerate a UTF-8 BOM (bytes pass through as-is otherwise; the
		'' buffer is treated as ASCII/ANSI text)
		if( env.inf.memlen >= 3 ) then
			if( (env.inf.memtext[0] = &hEF) andalso _
			    (env.inf.memtext[1] = &hBB) andalso _
			    (env.inf.memtext[2] = &hBF) ) then
				env.inf.mempos = 4
			end if
		end if
		env.inf.num = 0
	...
	'' but the parser still drives the ir/emit backend during cProgram(), so
	'' skipping only the Begin call crashes. Output suppression is done properly
	'' by the null IR backend (phase 1) — until then, this must run.
...
sub fbIncludeFile(byval filename as zstring ptr, byval isonce as integer)
	...
	'' fbcParser: include files always come from disk, even when the root
	'' source is memory-backed (the pop below restores the root's memtext)
	env.inf.memtext = NULL
	env.inf.memlen = 0
	env.inf.mempos = 0
