...
type FBFILE
	...
	'' fbcParser: memory-backed root source (fbc_compile_text). Non-NULL
	'' means the lexer reads from this buffer instead of file #num; include
	'' files are unaffected (fbIncludeFile clears memtext for the pushed
	'' context, and the include push/pop copies restore it). Positions are
	'' 1-based, mirroring the file seek() semantics the lexer expects.
	memtext         as const zstring ptr
	memlen          as integer
	mempos          as integer
end type
