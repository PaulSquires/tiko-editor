...
sub irInit( )
	'' fbcParser: all codegen backends were removed - every compile runs on
	'' the null backend (ir-null.bas), which discards emission entirely.
	'' The backend *option* is pinned to FB_BACKEND_GCC so the AST takes the
	'' high-level-backend code paths the null vtable is modeled on (its NULL
	'' slots are the ones those paths never call); a stray -gen in a #cmdline
	'' must not be able to route the AST onto the gas/gas64 paths.
...
function vregDumpToStr( byval v as IRVREG ptr ) as string
	...
	case IR_VREGTYPE_REG
		'' fbcParser: emitDumpRegName() went away with the gas backend
		''s += " reg="
		s += " " + str( v->reg )
	end select
