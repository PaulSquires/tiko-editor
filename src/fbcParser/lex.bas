		...
		if( (lex.ctx-1)->physfilepos > 0 ) then
			if( (lex.ctx-1)->physfilepos <> lex.ctx->physfilepos ) then
				if( env.inf.memtext <> NULL ) then
					'' fbcParser: memory-backed root source
					env.inf.mempos = (lex.ctx-1)->physfilepos
				else
...
private function hReadChar _
	...
	elseif( lex.ctx->kind = LEX_TKCTX_CONTEXT_EVAL ) then
		...
		if( lex.ctx->bufflen = 0 ) then
			'' fbcParser: memory-backed root source? (always ASCII format)
			if( env.inf.memtext <> NULL ) then
				if( env.inf.mempos <= env.inf.memlen ) then
					lex.ctx->filepos = env.inf.mempos
					dim as integer n = env.inf.memlen - env.inf.mempos + 1
					if( n > LEX_MAXBUFFCHARS ) then
						n = LEX_MAXBUFFCHARS
					end if
					dim as const zstring ptr src = @env.inf.memtext[env.inf.mempos-1]
					for i as integer = 0 to n-1
						lex.ctx->buff[i] = src[i]
					next
					env.inf.mempos += n
					lex.ctx->physfilepos = env.inf.mempos
					lex.ctx->bufflen = n
					lex.ctx->buffptr = @lex.ctx->buff
				end if

			elseif( eof( env.inf.num ) = FALSE ) then
...
function lexPeekCurrentLine _
	...
	if( env.inf.memtext <> NULL ) then
		'' fbcParser: memory-backed root source - same bytes "get" would
		'' read from 1-based file position p+1 (= 0-based offset p)
		dim as integer n = env.inf.memlen - p
		if( n > 1024 ) then
			n = 1024
		end if
		if( n < 0 ) then
