...
sub cAssignment( byval l as ASTNODE ptr )
	...
	'' fbcParser: mark the assignment TARGET as a write.
	''
	'' This cannot be done where the identifier is resolved: the whole LHS is
	'' parsed by cAssignmentOrPtrCall BEFORE it knows an '=' follows, so
	'' cVariableEx has no way to tell a read from a write and records both as
	'' reads. Here the '=' has just been consumed and the LHS is in hand as a
	'' tree, so the target is recoverable exactly.
	''
	'' The tree walk is why this is not a positional heuristic. "The first
	'' reference recorded while parsing the LHS" is wrong for `a(i).f = x`
	'' (that is `a`), and "the last" is wrong for `a(i) = x` (that is `i`).
	'' Walking down ->l lands on the FIELD for a member target and the VAR for
	'' an indexed one, which is what is actually being written.
	if( collWantsRefs( ) ) then
		...
		wend
		if( t <> NULL ) then
			'' a self-BOP (x += 1) genuinely reads AND writes
			collMarkLastRefWrite( t->sym, (op <> INVALID) )
		end if
	end if
