...
private function hParamDecl _
	...
	*id = ""
	...
	'' Add new param
	'' fbcParser: upstream discarded param names on prototypes (passed NULL
	'' when isproto) - they were irrelevant for codegen. The symbol extraction
	'' must report param names for DECLARE-only procs (header files!), so keep
	'' them. Param symbols aren't hashed, so this doesn't affect lookups; if a
	'' body follows, hCheckPrototype() re-sets the names from the body anyway.
	param = symbAddProcParam( proc, id, _
