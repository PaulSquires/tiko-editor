...
sub cProcDecl( )
	...
	case FB_TK_SUB, FB_TK_FUNCTION, FB_TK_OPERATOR
        ...
        dim sym as FBSYMBOL ptr '(added by me because orig source does not return proc symbol)
        dim as integer linenum = lexLineNum()

		'' ProcHeader
		sym = cProcHeader( 0, 0, FALSE, FB_PROCOPT_ISPROTO, tk )
        
'        tiko_ProcessSymbol(sym, linenum)
