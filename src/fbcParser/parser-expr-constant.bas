...
function cConstant( byval sym as FBSYMBOL ptr ) as ASTNODE ptr
	'' fbcParser: CONST and enum-value references. Ahead of the lexSkipToken
	'' below. Note this DOES fire for a const inside `#if X > 0` (ppExpression
	'' parses a real constant expression) which is correct - the const genuinely
	'' affects the build. `#ifdef X` does not reach here at all; it resolves
	'' through cIdentifierOrUDTMember in pp-cond.bas.
