...
function cFunctionCall _
	...
	'' fbcParser: proc references from EXPRESSION position.
	'' This is a parallel implementation of cProcCall, not a delegate - neither
	'' calls the other - so hooking only cProcCall counted statement-position
	'' calls and silently missed every `x = F(...)`, `print F(...)` and
	'' `dim y = F(...)`. Measured: a function called three times from
	'' expression position reported zero references.
	'' cMethodCall picks between the two by fbGetIsExpression(), so the two
	'' hooks are alternatives and cannot both fire for one call.
