...
private function hProcPtrBody _
	...
	'' fbcParser: TAKING A PROC'S ADDRESS IS A REFERENCE, and it does NOT go
	'' through cProcCall - `@MyProc` reaches here via cAddrOfExpression, as does
	'' PROCPTR(). Without this hook every callback (WndProc, thread proc,
	'' comparison function) reads as never referenced, which is the single
	'' biggest false-positive class in a Win32 codebase.
	'' Placed after overload resolution and ahead of all the early returns
	'' below, so every path that gets this far is counted exactly once.
	if( collWantsRefs( ) ) then
		collRecordRef( proc, FALSE )
	end if
