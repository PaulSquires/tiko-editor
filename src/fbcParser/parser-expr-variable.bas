...
#include once "ast.bi"
#include once "collector.bi"    '' fbcParser: reference recording
...
private function hMemberId( byval parent as FBSYMBOL ptr, byval allow_inner as integer ) as FBSYMBOL ptr
	...
	'' fbcParser: member references (a.b, p->b, WITH .b, implicit-THIS fields).
	'' This is the ONLY route by which a FIELD is ever referenced, so without
	'' it every field in every TYPE reads as unused.
	'' PROCs are deliberately skipped: a method reference always goes on to
	'' cProcCall or hProcPtrBody, and recording it here too would count every
	'' obj.Method() twice. Still ahead of the caller's lexSkipToken.
	if( collWantsRefs( ) ) then
		if( symbGetClass( sym ) <> FB_SYMBCLASS_PROC ) then
			collRecordRef( sym, FALSE )
		end if
	end if
...
function cVariableEx overload _
	...
	'' fbcParser: every source-text variable reference funnels through here -
	'' reads, assignment targets, field/array bases, @addr, BYREF args, WITH
	'' heads. Recorded as a READ; cAssignment upgrades the target to a write
	'' one token later. MUST stay ahead of the lexSkipToken below, or
	'' lexLineNum() has already moved off the identifier.
	'' (The chain-taking cVariableEx overload delegates here, so hooking it
	'' too would count every reference twice.)
	if( collWantsRefs( ) ) then
		collRecordRef( sym, FALSE )
	end if
