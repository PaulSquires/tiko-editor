				...
				do while( chain_ <> NULL )
					...
					do
						...
						case FB_SYMBCLASS_PROC, FB_SYMBCLASS_LABEL
							sym = s
							'' fbcParser: asm operand (LABELs are never
							'' a collected kind, so only PROC lands)
							if( collWantsRefs( ) ) then
								collRecordRef( s, FALSE )
							end if
							exit do, do
						...
						case FB_SYMBCLASS_VAR
							...
							if( sym <> NULL ) then
								...
								'' fbcParser: always a READ, even for
								'' `mov [x], eax`. An asm write is not
								'' distinguishable here, and reporting it
								'' as a read makes an asm-written variable
								'' come out "read, never written" instead
								'' of dead - the safe direction.
								if( collWantsRefs( ) ) then
