...
private sub hCheckPrototype _
	...
	while( (i <= proto_params) and (i <= params) )
		...
		if( param->param.mode <> FB_PARAMMODE_VARARG ) then
			...
			'' fbcParser: the body's param is the one whose name the proto
			'' param now carries - take its declaration location and its
			'' original-case name too (sharing the ptr is safe: orignames
			'' are pool-allocated and never individually freed)
			proto_param->defloc = param->defloc
			proto_param->origname = param->origname
