...
function cProcCall _
	...
	'' fbcParser: proc references, from both statement and expression position.
	'' Recorded before any argument is parsed - arguments carry their own
	'' references and move lexLineNum() off the call.
	'' NOTE sym is the overload HEAD, not the resolved overload, which is why
	'' hRefTrackedFlag() reports a whole overloaded set as unknown rather than
	'' crediting the head and calling every sibling dead.
