		...
		do
			...
			case FB_TK_COMMENT
				if( hasParens ) then
					...
					'' Consume the rest of the comment with the same raw reader
					'' this loop already uses, so that lex.ctx->head is left
					'' untouched. lexSkipToken() would pre-fetch the token after
					'' the EOL into head, and the macro text installed into
					'' lex.ctx->deftext below then overwrites it -- silently
					'' eating the first token of the next line, and counting the
					'' EOL twice (once here, once for the re-added CHAR_LF).
					do
		...
		do
			...
			case FB_TK_COMMENT
				if( hasParens ) then
					...
					'' See the matching comment in hLoadMacro(): skipping tokens
					'' here would pre-fetch (and then lose) the first token of
					'' the next line, and double-count the EOL.
					do
						lexNextToken( @t, LEX_FLAGS or LEXCHECK_NOMULTILINECOMMENT )
					loop until( (t.id = FB_TK_EOL) or (t.id = FB_TK_EOF) )
