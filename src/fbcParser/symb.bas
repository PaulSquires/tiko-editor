...
'':::::
'' fbcParser: find the 1-based column of identifier id in linetext, matching
'' whole words only (so a decl like "dim n as long" doesn't match the 'n' in
'' "long" of an unrelated earlier position). 0 if not found.
private function hFindIdentifierColumn _
	( _
		byref linetext as string, _
		byval id as const zstring ptr _
	) as integer

	dim as string uline, uid
	dim as integer p, idlen, atstart, atend
	...
	uline = ucase( linetext )
	uid = ucase( *id )

	p = instr( uline, uid )
	do while( p > 0 )
		'' whole word? check the chars around the match (identifier
		'' chars are A-Z, 0-9, '_')
		atstart = TRUE
		if( p > 1 ) then
			select case( uline[p-2] )
		...
		atend = TRUE
		if( p + idlen <= len( uline ) ) then
			select case( uline[p+idlen-1] )
			case asc( "A" ) to asc( "Z" ), asc( "0" ) to asc( "9" ), asc( "_" )
				atend = FALSE
			end select
		end if
		...
		p = instr( p + 1, uline, uid )
	loop

end function

'' fbcParser: record where a symbol is being declared, from the lexer's
'' current state. Called once per created symbol from symbNewSymbol().
private sub hSetDefLocation _
	( _
		byval s as FBSYMBOL ptr, _
		byval id as const zstring ptr _
	)

	'' Cache the peeked line: all symbols created while the lexer sits on
	'' the same line of the same file (e.g. every param of a declare) share
	'' it, and lexPeekCurrentLine() re-reads the source file each call.
	static as zstring ptr cachedfile = NULL
	static as integer cachedline = -1
	static as string cachedtext
	...
	'' Nothing to record for symbols created outside of parsing: keywords,
	'' intrinsic defines and rtl procs are registered during fbInit, before
	'' any source file is open. A memory-backed root source (fbc_compile_text)
	'' has num = 0 but memtext set - that IS active parsing.
	if( env.inited = FALSE ) then
		exit sub
	end if
	if( (env.inf.num <= 0) andalso (env.inf.memtext = NULL) ) then
		exit sub
	end if

	'' Capture the identifier's original casing - id is the raw source text,
	'' before symbNewSymbol ucases it into id.name.
	if( id <> NULL ) then
		dim as integer slen = len( *id )
		if( slen > 0 ) then
			s->origname = poolNewItem( @symb.namepool, slen + 1 )
			*s->origname = *id
		end if
	end if

	s->defloc.file = env.inf.incfile
	s->defloc.linenum = lexLineNum( )

	'' Best-effort column: locate the identifier in the current line's
	'' text. The parser has usually consumed the identifier (and more) by
	'' the time the symbol is created, so the lexer's own token position
	'' can't be used directly. See FBSYMBOL_DEFLOC for the cases where
	'' this stays 0.
	if( id <> NULL ) then
		if( (cachedfile <> s->defloc.file) orelse (cachedline <> s->defloc.linenum) ) then
			dim as string token_pos
			cachedtext = lexPeekCurrentLine( token_pos, FALSE )
			cachedfile = s->defloc.file
			cachedline = s->defloc.linenum
		end if
		s->defloc.colnum = hFindIdentifierColumn( cachedtext, id )
	end if
...
add_prev:       head_sym->hash.item->data = s
	...
	'' fbcParser: record the declaration location (after the dup check, so
	'' only symbols that actually survive get stamped)
	hSetDefLocation( s, id )
