...
''
'' fbcParser: where a symbol was declared in the scanned source.
'' file is the interned filename ptr from env.filenamehash (env.inf.incfile),
'' valid until fbEnd() - the phase-3 collector converts it into a file-table
'' index while marshalling. NULL for compiler-created symbols (keywords,
'' intrinsic defines/procs, temporaries). linenum/colnum are 1-based; 0 means
'' unknown. colnum is best-effort: 0 for UTF-16/32 sources, for identifiers on
'' an earlier line of a multi-line (_-continued) declaration, and inside macro
'' expansions (where linenum points at the invocation site).
type FBSYMBOL_DEFLOC
	file            as zstring ptr
	linenum         as integer
	colnum          as integer
end type
...
type FBSYMBOL
	...
	defloc          as FBSYMBOL_DEFLOC          '' fbcParser: declaration location

	'' fbcParser: the identifier exactly as written in the source, captured
	'' before id.name gets ucased. Allocated from symb.namepool and never
	'' individually freed (the pool dies at symbEnd) - so unlike id.name,
	'' which symbSetName() poolDelItem's on rename, this ptr may be safely
	'' shared between symbols. NULL for compiler-created/anonymous symbols.
	origname        as zstring ptr
